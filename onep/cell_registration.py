...
class CellReg:
    ...
    def load_registration_table(self, filter_accepted=True):
        ...
        self.registration_table = self.registration_table.iloc[
            ~(self.registration_table == -1000).all(axis='columns').values]
    ...
    def load_footprints_3D(self, select_sessions=False, affine_shifted=False):
        ...
        '''
        if select_sessions:
            self.footprint_files = filedialog.askopenfilenames(
                title="Select the footprint files in chronological order of sessions")
            for i in range(self.N_sessions):
                foot_file = filedialog.askopenfilename()  ##CLICK THE FILES IN CHRONO ORDER!
                self.footprint_files.append(foot_file)
        else:
            sessions = self.metadata['Session'].loc[
                (self.metadata['Animal'] == self.animal) & (self.metadata['FOV'] == self.FOV)].values
        ...
        if affine_shifted:
            footprint_path = os.path.join(self.base_directory, 'CellReg', self.animal + '_' + self.FOV,
                                          'shifted_footprints')
            footprint_files = [
                os.path.join(footprint_path, self.animal + '_' + self.FOV + '_' + session + '_shifted_footprints.mat')
        ...
        self.footprints = [self.convert_foots_to_masks(foots_float[i]) for i in range(
            len(foots_float))]  # must convert to masks if imported footprints from inscopix helper files.
    ...
    def load_shifted_footprints_2D(self):
        ...
        ** this includes all the cells given to CellReg including Isx rejected cells **
        ...
        '''
        aligned_map_file = h5py.File(
            os.path.join(self.base_directory, 'CellReg', self.animal + '_' + self.FOV, 'aligned_data_struct.mat'))
        aligned_struct = aligned_map_file['aligned_data_struct']
        self.footprints_reg = []
        for i in range(self.N_sessions):
            foot_aligned = aligned_map_file[aligned_struct['footprints_projections_corrected'][i, 0]][()].transpose(
                (1, 0))
    ...
    def get_reg_ind(self):
        ...
        reg_path = os.path.join(self.base_directory,
                                'CellReg' + os.path.sep + self.animal + '_' + self.FOV + os.path.sep)
        reg_file = [os.path.join(reg_path, f) for f in os.listdir(reg_path) if 'cellRegistered' in f]
        file = h5py.File(reg_file[-1],
                         'r')  # chooses the last cellreg output file in the directory, make sure theres only 1 present!
    ...
    def export_affine_shift_footprints(self, footprints_all, X_shifts, Y_shifts, Rotations, Shears, session_inds=None):
        ...
        for i in range(num_sessions):
            ...
            savemat(os.path.join(savepath, self.animal + '_' + self.FOV + '_' + session + '_shifted_footprints.mat'),
                    {'footprints_shifted': shifted, 'shift_x': shift_x, 'shift_y': shift_y, 'rotation': rotation,
                     'shear': shear})

    def export_affine_shift_images(self, images, X_shifts, Y_shifts, Rotations, Shears):
        im_path = os.path.join(self.base_directory, 'Summary_Images', self.animal + '_' + self.FOV,
                               'affine_shifted_images')
        ...
        for i, im in enumerate(images):
            ...
            tifffile.imwrite(os.path.join(im_path, self.animal + '_' + self.FOV + '_' + session + '_affine_shift.tif'),
                             im_shifted)
    ...
    def rois_plot(self, session_ind, idxs=None, image=None, min_pct='default', max_pct='default', scale_factor: int = 2,
        ...
        if image is not None:
            ...
            plot = hv.Image(image).opts(cmap=cmap_image, clim=clim, width=int(dims[1] * scale_factor),
                                        height=int(dims[0] * scale_factor), xaxis=None, yaxis=None) * hv.Image(
                sum_masks).opts(cmap='hsv',
                                alpha=.5)
        else:
    ...
    def plot_im_stack(self, images, titles=None, cmap='gray', scale_factor=3):
        if titles == None:
            im_dict = {
                i: hv.Image(im).opts(width=int(im.shape[1]) * scale_factor, height=int(im.shape[0]) * scale_factor,
                                     cmap=cmap, xaxis=None, yaxis=None) for i, im in enumerate(images)}
        else:
            im_dict = {titles[i]: hv.Image(im).opts(width=int(im.shape[1]) * scale_factor,
                                                    height=int(im.shape[0]) * scale_factor, cmap=cmap, xaxis=None,
                                                    yaxis=None) for i, im in enumerate(images)}
