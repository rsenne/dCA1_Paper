...
# ╔═╡ 6d1dfc96-1a8a-4fb8-ac54-75fbb368bf71
begin  
...
# ╔═╡ ec8d32a0-508e-11f0-2868-27818e766d61
md"# High Activity Event Detector"

# ╔═╡ d1a14cce-dafe-4f20-bbbf-1ccd81234cc4
md"This Pluto notebook houses the code used for detecting putative sequences in the paper: *A hippocampal astrocytic sequence emerges during learning and memory.*" 

# ╔═╡ 14eb3517-90e8-4010-9cfa-7fe7b1412dd4
md"## This cell loads data and defines some useful functions for later on."

# ╔═╡ 1feb1da4-bfc5-43f3-8b79-dd34b97a1eae
begin
	# specify where the files are located
	dir = "/Users/ryansenne/Desktop/Dill"
	ddir = joinpath(dir, "hab_traces_csv")
	
	# get the list of all .csv files containing "astro" in the name
	csv_files = filter(f -> endswith(f, "accepted_traces.csv") && occursin("astro", f), readdir(ddir))
...
begin
	file_choices = collect(keys(results))
	@bind selected_file PlutoUI.Select(file_choices)
end

# ╔═╡ dedf52e8-5d39-4ce5-9cd8-1d04ae7e8d72
begin
	function plot_file_results(file_key::String)
	    ...
	    for seq in events
	        xspan = time[seq[1]]:time[seq[end]]
	        yspan = maximum(data) * ones(length(xspan))
	        plot!(xspan, yspan, fillrange=0, fillalpha=0.25, color=:red, label=false)
	    end
...
manifest_format = "2.0"
project_hash = "c223c203ce44d792528341bec803e0bc39e23e7d"
