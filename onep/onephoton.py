...
__all__ = ["InscopixProcessing", "dCA1Group", "maxsort", "cross_validated_heat_plot", "sequence_heat_plot",
...
class InscopixProcessing():
    ...
    def get_DLC_data(self, data='freezing'):
        ...
        DLC_file = os.path.join(DLC_path,
                                'Astro_' + ani_id + '_' + self.session.upper() + '_VideoDLC_resnet50_OLMMay26shuffle1_300000.csv')
...
def cosine_similarity_matrix(arr, time, cmap, seq_times=[120, 180, 240, 300]):
    ...
    # plot the heatmap
    sns.heatmap(cos_sim, cmap=cmap, cbar=True, cbar_kws={"label": "Cosine Similarity"}, rasterized=True, vmin=-1,
                vmax=1, ax=ax)
